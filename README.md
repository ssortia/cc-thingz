...
- **Шаг 3** — открывает созданный план в хостовой JetBrains IDE (через `scripts/open-in-ide.sh`, best-effort — определяет IDE по дереву процессов, вне JetBrains-терминала молча ничего не делает), затем предлагает интерактивное ревью (открывает план в `$EDITOR` через plan-annotate), авто-ревью, начало реализации или завершение
...
1. **Цикл задач** — один субагент на секцию задачи, коммит после подтверждения пользователем, повтор при сбое
2. **Комплексное ревью** — 6 параллельных агентов (качество, реализация, тестирование, упрощение, документация, запахи кода) + fixer; на повторных critical-проверках ревью сужается до файлов, изменённых фиксером, и их прямых потребителей
3. **Внешнее ревью** — автоопределение CLI `codex` или использование пользовательской команды, состязательный цикл с ранним выходом по уровню серьёзности (останавливается после первой итерации, не нашедшей критических/серьёзных проблем; минорные находки всё равно исправляются)
4. **Ревью только критичного** — 2 агента (качество + реализация), только критические/серьёзные проблемы + fixer; пропускается, если со времени фазы 2 ничего не закоммичено
5. **Финализация** — rebase, squash, проверка (опционально)
6. **Сводка статистики** — один агент читает лог сессии + состояние git и сообщает суммарные токены / общее время / разбивку по фазам / churn веток / итерации fixer
...
**Подтверждение коммитов** — субагенты (task и fixer) сами не коммитят: они оставляют изменения в рабочем дереве и возвращают `SUMMARY` (что и зачем сделано) и список `FILES`. Оркестратор описывает изменение и спрашивает подтверждение (Approve / Request changes / Abort) перед коммитом через `stage-and-commit.sh`, чтобы пользователь успел просмотреть дифф в своей IDE. Гейт действует и в автономном режиме.
...
| `review_iterations` | `3` | Макс. число циклов исправления-и-перепроверки при внутреннем ревью |
| `external_review_iterations` | `5` | Макс. число итераций состязательного цикла внешнего ревью |
